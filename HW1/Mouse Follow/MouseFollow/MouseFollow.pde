...
void update(float dt){
  ...
  if((mousePos.minus(pos)).length() <= speed*0.01){
    dir = new Vec2(0, 0); //Should be vector pointing from pos to 
    //dir.setToLength(0); <=== why doesn't this work?
    pos = mousePos;
  }
...
void draw(){
  ...
  circle(pos.x, pos.y,r*2); //Question: Why is it radius*2 ?
                            //Answer: The radius is only half of the total diameter, and the circle function is expecting the total width/height of the circle, aka the diameter, not the radius.
}
